...
void LZMAStream::AsyncCode(const FunctionCallbackInfo<Value>& args) {
	...
	if (self->hasRunningThread) {
		isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "Can only have one running thread per stream")));
		args.GetReturnValue().SetUndefined();
		return;
	}
	...
	pipe_create_failure:
		isolate->ThrowException(Exception::Error(String::NewFromUtf8(isolate, "Could not create pipe")));
		args.GetReturnValue().SetUndefined();
		return;
	}
	...
	ret->Set(0, Integer::New(isolate, fromCompressor[0]));
	ret->Set(1, Integer::New(isolate, toCompressor[1]));
